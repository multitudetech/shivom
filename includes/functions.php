...
function sql($sql, $con = "") {
    ...
    $result = mysql_query($sql, $con); // or oops(mysql_error());
    if ($result <> 1) {
...
//	$data = _update($data);
    ...
    return mysql_query_custom($q, $con);
}
...
function backup_tables($tables = '*') {
    ...
    foreach ($tables as $table) {
        ...
        $row2 = mysql_fetch_row(mysql_query('SHOW CREATE TABLE ' . $table, $dbh));
        $return.= $row2[1] . ";";
